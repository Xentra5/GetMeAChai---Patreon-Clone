...
export default function SignPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            // OTP request succeeded. Show verification screen and store token.
    ...
    const handleVerifyOtp = async (event) => {
        ...
        try {
            const response = await fetch("/api/signup", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ signupToken, otp: otpCode }),
            });
            ...
            if (result?.error) {
                setError("Account verified, but automatic login failed: " + result.error);
                setLoadingVerify(false);
                return;
            }
        ...
        <section className="min-h-[calc(100vh-64px)] flex items-center justify-center text-white py-12 px-4">
            <div className="w-full max-w-md">
                ...
                <h1 className="mb-2 text-center text-4xl font-extrabold text-white">
                    {showOtpScreen ? "Verify Email" : "Sign Up"}
                </h1>
                <p className="mb-8 text-center text-gray-400 text-sm">
                    {showOtpScreen ? `We sent a 6-digit verification code to ${email}` : "Create your account to get started"}
                </p>
                ...
                <div className="relative rounded-2xl border border-slate-800 bg-[#121316] p-8 shadow-xl">
                    ...
                    {showOtpScreen ? (
                        ...
                        <form onSubmit={handleVerifyOtp} className="space-y-6">
                            ...
                            <div>
                                <label htmlFor="otp" className="mb-2 block text-center text-sm font-medium text-white/70">
                                    Enter 6-digit code
                                </label>
                                <input
                                    id="otp"
                                    ...
                                    className="w-full text-center text-2xl tracking-[8px] font-mono rounded-lg border border-slate-800 bg-[#090a0f] px-4 py-3 text-white placeholder-gray-600 outline-none transition duration-200 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full rounded-lg bg-purple-600 px-4 py-3 font-semibold text-white transition-all duration-200 hover:bg-purple-700 hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-purple-500/10 disabled:opacity-50"
                            >
                                {loadingVerify ? "Verifying..." : "Verify & Complete Signup"}
                            </button>
